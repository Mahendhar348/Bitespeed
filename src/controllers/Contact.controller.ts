...
export const postContacts = async (req: any, res: any) => {
    try {
        ...
        if (firstRecord && firstRecord.id != record.id) {
            await Contacts.updateMany({ id: { $ne: firstRecord.id }, $or: [{ email: reqPayload.email }, { phoneNumber: reqPayload.phoneNumber }] }, { $set: { "linkPrecedence": "secondary", "linkedId": firstRecord.linkPrecedence === "primary" ? firstRecord.id : firstRecord.linkedId, updatedAt:Date.now() } });
        }
